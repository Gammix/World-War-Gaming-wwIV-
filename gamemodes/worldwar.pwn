...
main()
{
	...
	print("\n============================================");
	print("* World War Gaming community - Build  "#BUILD"");
	print("* Owned by Gammix and others | Created by Gammix");
	print("* ------------");
...
public OnGameModeInit()
{
	...
	{
	    gTeam[i][teamBaseId] = GangZoneCreate(gTeam[i][teamBase][0], gTeam[i][teamBase][1], gTeam[i][teamBase][2], gTeam[i][teamBase][3], SET_ALPHA(gTeam[i][teamColor], 100), 2.0, COLOR_RED);
	...
	{
	    gZone[i][zoneId] = GangZoneCreate(gZone[i][zonePos][0], gZone[i][zonePos][1], gZone[i][zonePos][2], gZone[i][zonePos][3], SET_ALPHA(gTeam[gZone[i][zoneOwner]][teamColor], 100), 2.0);
...
public OnPlayerSpawn(playerid)
{
	...
	else
	{
	    ...
	    if (pStats[playerid][userPremium])
	    {
	        gClass[pClass[playerid]][classWeapon1][1] *= 3;
	        gClass[pClass[playerid]][classWeapon2][1] *= 3;
	        gClass[pClass[playerid]][classWeapon3][1] *= 3;
	        gClass[pClass[playerid]][classWeapon4][1] *= 3;
	        gClass[pClass[playerid]][classWeapon5][1] *= 3;
	    }
		GivePlayerWeapon(playerid, gClass[pClass[playerid]][classWeapon1][0], gClass[pClass[playerid]][classWeapon1][1]);
		GivePlayerWeapon(playerid, gClass[pClass[playerid]][classWeapon2][0], gClass[pClass[playerid]][classWeapon2][1]);
		GivePlayerWeapon(playerid, gClass[pClass[playerid]][classWeapon3][0], gClass[pClass[playerid]][classWeapon3][1]);
		GivePlayerWeapon(playerid, gClass[pClass[playerid]][classWeapon4][0], gClass[pClass[playerid]][classWeapon4][1]);
		GivePlayerWeapon(playerid, gClass[pClass[playerid]][classWeapon5][0], gClass[pClass[playerid]][classWeapon5][1]);
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if (0 <= GetPlayerTeam(playerid) < MAX_TEAMS)
	{
	    switch (GetVehicleModel(GetPlayerVehicleID(killerid)))
	    {
	   		...
	   		{
				if (IsPlayerInGangZone(playerid, gTeam[GetPlayerTeam(playerid)][teamBaseId]))
				{
	...
	foreach (new i : Player)
	{
		...
		else if (pCarepack[i][cpCalled])
    	{
			...
			else if (objectid == pCarepack[i][cpObject])
			{
			    ...
			    pCarepack[i][cpLabel] = CreateDynamic3DTextLabel(text, SET_ALPHA(COLOR_GREEN, 100), x, y, z, 50.0, .worldid = 0);
...
CMD:mine(playerid)
{
	...
	for (new i; i < 3; i++)
	{
		...
		{
			pLandmineObject[playerid][i] = CreateDynamicObject(19602, x, y, z - 0.9, 0.0, 0.0, 0.0, 0, GetPlayerInterior(playerid));
...
CMD:dropgun(playerid)
{
	...
	switch (w)
	{
		case 1..15:
	    {
			CreateWeaponPickup(w, 1, x, y + random(4), z, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
			GivePlayerWeapon(playerid, w, -1);
	    }
	    case 16..37, 39..46:
	    {
	    ...
	    default:
	    {
	        return SendClientMessage(playerid, COLOR_TOMATO, "You cannot drop this weapon or it's unequipable!");
	    }
	}
